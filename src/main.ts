...
function commandHandler(input: string) {
  ...
  switch (input) {
    ...
    case 'banner':
      if (bareMode) {
        writeLines(["System x86_64 v1.0.0", "<br>"])
        break;
      }
      writeLines(BANNER);
      break;
    ...
    case 'whereis --github':
      writeLines(["Redirecting...", "<br>"]);
      setTimeout(() => {
        window.open("https://www.github.com/" + SOCIAL.github, '_blank');
      }, 500);
      break;
    ...
    case 'whereis --x':
      writeLines(["Redirecting...", "<br>"]);
      setTimeout(() => {
        window.open("https://www.x.com/" + SOCIAL.x, '_blank');
      }, 500);
      break;
    ...
    case 'email':
      writeLines(["Redirecting...", "<br>"]);
      setTimeout(() => {
        window.open("mailto:" + SOCIAL.email, '_blank');
      }, 500);
      break;
...
function passwordHandler() {
  if (passwordCounter === 2) {
    ...
    writeLines(["<br>", "Incorrect password.", "<br>"])
...
const initEventListeners = () => {
  ...
  console.log(`%cPassword: ${command.password}`, "color: red; font-size: 9px;");
}
